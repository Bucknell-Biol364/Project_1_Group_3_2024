...
ggplot(temp)+  #The name of the data frame you are using goes in the parenthesis
...
lmbee<-glm(colony_lost_pct~year, data=temp)
summary(lmbee)
...
```{r}
lmbeelog<-glm(log(colony_lost_pct)~year, data=temp)
summary(lmbeelog)
plot(lmbeelog)
```
...
Unfortunately, it appears that when p-values are adjusted to account for multiple comparisions, none of the years are significantly different. However, we can also improve the linear model by adding addition explantory variables, such as the state and months. 



```{r}
lmbeenew<-glm(log(colony_lost_pct)~year + state + months, data=temp)
summary(lmbeenew)
plot(lmbeenew)
```
